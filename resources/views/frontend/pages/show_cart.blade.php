								...
								<div class="cart_quantity_button">
									<form action="/gio-hang/cap-nhat" method="post">
										...
										<input type="hidden" name="rowId" value="{{$ct->rowId}}"> 
										 ...
										 <button type="button" class="btn btn-warning cart_quantity_up" data-qty_product="{{$ct->qty}}" data-id_product="{{$ct->id}}"
										data-row_id="{{$ct->rowId}}" >+</button> 

										<input 
										class="cart_quantity_id_{{$ct->id}} btn btn-light" 
										type="number" name="quantity" value="{{$ct->qty}}"  >

										<button type="button" class="cart_quantity_down btn btn-warning"  data-qty_product="{{$ct->qty}}" data-id_product="{{$ct->id}}"
										data-row_id="{{$ct->rowId}}">-</button>
										<input type="submit" value="Cập nhật" name="update_qty" class="btn btn-warning"> 
									</form>
						...
						<div class="chose_area">
							...
							<table>
								<thead class="thead-dark">
										<tr>
											<th><h3><span class="label label-success">Mã giảm giá</span></h3></th>
											<th scope="col"></th>
											<th scope="col"></th>
...
<script>
        $(document).ready(function(){
            $(".cart_quantity_up").click(function(){
                ...
                $.get("ajax/qty/up/"+rowId+"/"+id+"/"+qty,function(data){
            ...
            $(".cart_quantity_down").click(function(){
                ...
                var rowId = $(this).data('row_id');
                //alert(rowId);
                $.get("ajax/qty/down/"+rowId+"/"+id+"/"+qty,function(data){
                    $(".cart_quantity_id_"+id).val(data);
                    //alert(data);
                    location.reload();
                });
