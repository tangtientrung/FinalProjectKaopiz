...
class AjaxController extends PageController
{
	...
	{
			$quan_huyen=DistrictModel::where('matp',$id)->get();
	...
	{
			$xa_phuong_thi_tran=WardsModel::where('maqh',$id)->get();
	...
	{
		$fee=FeeShipModel::where('fee_matp',$matp)->where('fee_maqh',$maqh)->where('fee_xaid',$xaid)->first();
		if($fee)
		{
...
';
	...
	{
		$sub_category=SubCategoryModel::where('category_id',$id)->get();
